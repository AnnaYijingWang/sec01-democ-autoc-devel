...
```{r}
anrr = read.csv("data/section1_df.csv")
```
...
```{r}
ggplot(anrr) + 
  geom_histogram(aes(x=gdppc, fill = dem_label))
```
...
```{r}
ggplot(anrr) + 
  geom_boxplot(aes(x=gdppc, fill=dem_label)) +
  scale_x_log10(labels = scales::dollar)
```
